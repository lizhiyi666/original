...
采样过程中，投影方法会根据 `projection_frequency` 参数按指定频率应用，确保生成的 POI 序列满足偏序约束。
...
- **默认**: 1
- **说明**: 每隔几步应用一次投影（1 表示每步都应用）
- **建议**:
  - 1: 每步都应用投影（最严格，最慢）
  - 2-5: 每隔几步应用一次（平衡速度和约束满足）
  - 10+: 较少应用投影（快速，但约束满足率可能降低）
- **使用场景**:
  - 训练后期/高质量采样: 设为 1
  - 快速采样/初步测试: 设为 5-10
  - 仅在关键步骤约束: 可以结合其他策略自定义
