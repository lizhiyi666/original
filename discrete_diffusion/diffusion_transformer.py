...
def alpha_schedule(time_step, att_1=0.99999, att_T=0.000009, ctt_1=0.000009, ctt_T=0.99999, type_classes=9,
                   poi_classes=381):
    sep = 5
    sep_1 = sep - 1
    att = np.concatenate(
        (np.arange(0, time_step * sep_1 // sep) / (time_step * sep_1 // sep - 1) * (0.0001 - 0.99999) + 0.99999,
         np.arange(0, time_step - time_step * sep_1 // sep) / (time_step - time_step * sep_1 // sep - 1) * (
                     0.000009 - 0.00009) + 0.00009))
    att = np.concatenate(([1], att))
    at = att[1:] / att[:-1]

    att1 = np.concatenate(
        (np.arange(0, time_step * sep_1 // sep) / (time_step * sep_1 // sep - 1) * (0.9999 - 0.99999) + 0.99999,
         np.arange(0, time_step - time_step * sep_1 // sep) / (time_step - time_step * sep_1 // sep - 1) * (
                     0.000009 - 0.9999) + 0.9999))
    att1 = np.concatenate(([1], att1))
    at1 = att1[1:] / att1[:-1]

    ctt = np.concatenate(
        (np.arange(0, time_step * sep_1 // sep) / (time_step * sep_1 // sep - 1) * (0.00009 - 0.000009) + 0.000009,
         np.arange(0, time_step - time_step * sep_1 // sep) / (time_step - time_step * sep_1 // sep - 1) * (
    ...
    at = np.concatenate((at[:time_step * sep_1 // sep], (1 - ct - bt * poi_classes)[time_step * sep_1 // sep:])).clip(
        min=1e-30)
...
class ConditionEmbeddingModel(nn.Module):
    ...
    def forward(self, batch, force_dropout=False):  # [修改] 增加参数
        ...
        raw_cond_emb = torch.cat([time_embeddings, condition1_embeddings, condition2_embeddings, \
                                  condition3_embeddings, condition4_embeddings, condition5_embeddings,
                                  condition6_embeddings], dim=-1)
        ...
        if force_dropout:
            ...
            mask = torch.bernoulli(
                torch.ones(batch.batch_size, 1, 1, device=batch.time.device) * (1 - self.dropout_rate))
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        self.transformer = Transformer(tgt_vocab_size=self.num_classes, num_spectial=self.num_spectial,
                                       type_classes=self.type_classes, poi_classes=self.poi_classes)
        ...
        at, at1, bt, bt1, ct, ct1, att, att1, btt1, btt2, ctt, ctt1 = alpha_schedule(self.num_timesteps,
                                                                                     type_classes=self.type_classes,
                                                                                     poi_classes=self.poi_classes)
    ...
    def q_pred_one_timestep(self, log_x_t, t, batch):  # q(xt|xt_1) [-2] mask_category [-1] mask_poi
        ...
        log_x_start_category = torch.cat(
            [log_x_start_category[:, :-2], log_add_exp(log_x_start_category[:, -2:-1], log_ct1),
    ...
    def p_pred(self, log_x, cond_emb, t, batch, uncond_emb=None,
               guidance_scale=0.0):  # if x0, first p(x0|xt), then sum(q(xt-1|xt,x0)*p(x0|xt))
    ...
    def p_sample(self, log_x, cond_emb, t, batch, po_constraints=None, diffusion_index=None,
        ...
        model_log_prob, log_x_recon = self.p_pred(log_x, cond_emb, t, batch, uncond_emb=uncond_emb,
                                                  guidance_scale=guidance_scale)
        ...
        if should_apply_projection:
            if getattr(self, "debug_constraint_projection", False) and not getattr(self, "_debug_projection_printed",
                                                                                   False):
                self._debug_projection_printed = True
                print("[DEBUG][projection] CALLED in p_sample")
                print(
                    f"[DEBUG][projection] diffusion_index={diffusion_index}, projection_frequency={self.projection_frequency}")
                print(f"[DEBUG][projection] model_log_prob.shape={tuple(model_log_prob.shape)}")
                print(
                    f"[DEBUG][projection] category_mask.sum={batch.category_mask.sum().item() if hasattr(batch, 'category_mask') and batch.category_mask is not None else None}")
                print(f"[DEBUG] Projection triggered at step {diffusion_index}")

            debug_viol = getattr(self, "debug_constraint_projection", False) and not getattr(self,
                                                                                             "_debug_viol_printed",
                                                                                             False)
            if debug_viol:
            ...
            if debug_viol:
                print(
                    f"[DEBUG] Constraint Type: {'Per-Sample (List of Lists)' if is_per_sample else 'Shared (List of Tuples)'}")
            ...
            if debug_viol and W_A is not None:
                with torch.no_grad():
                    ...
                    print(
                        f"[DEBUG][projection] viol_before[0]={viol_before[0].item():.6f}, mean={viol_before.mean().item():.6f}")
            ...
            if debug_viol and W_A is not None:
                with torch.no_grad():
                    ...
                    print(
                        f"[DEBUG][projection]  viol_after[0]={viol_after[0].item():.6f}, mean={viol_after.mean().item():.6f}")
    ...
    def training_losses(
        ...
        losses = {}
        loss = F.cross_entropy(log_x0_recon.reshape(-1, log_x0_recon.shape[-1]), x_start.flatten(), ignore_index=3,
                               reduce=False)
